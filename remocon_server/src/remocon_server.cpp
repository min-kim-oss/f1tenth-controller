...
RemoconServer::RemoconServer(const rclcpp::NodeOptions & options)
  :  rclcpp::Node("remocon_server", options) 
{
    ...
    // Timer
    {
        ...
        timer_control_ = rclcpp::create_timer(
          this,get_clock(), period_ns, std::bind(&RemoconServer::timerCallBack, this));
    }

    addr_server = {};
    addr_client = {};

    std::thread t1(&RemoconServer::socketTread, this);
    t1.detach();
}


void RemoconServer::SocketSetter()
{
    char port[6] = "9000";
...
void RemoconServer::socketTread(){
    SocketSetter();
    while(true){
        acceptingClient();
        ReceiveKey();
...
void RemoconServer::acceptingClient()
{
...
void RemoconServer::keyHandler(char r_key){
    switch(r_key)
    {
        case KEYCODE_W:
...
void RemoconServer::timerCallBack()
{
    hitRecoverer();
}

void RemoconServer::hitRecoverer()
{
...
void RemoconServer::cmd_creator(int accel_hitCount, int steer_hitCount, int brake_hitCount)
{
...
void RemoconServer::cmd_publisher(float cur_accel_arg, float cur_steer_arg, float cur_brake_arg)
{
