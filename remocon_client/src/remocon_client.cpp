...
RemoconClient::RemoconClient(std::shared_ptr<rclcpp::Node> nh, char* ip, char* port, struct termios* cooked, struct termios* raw) : nh_(nh)
{
    //setting addr struct
    addr_vehicle = {};
...
RemoconClient::~RemoconClient()
{
    tcsetattr(kfd, TCSANOW, cooked);
    
}

void RemoconClient::keyLoop()
{
    char c;
    bool key_dirty;
...
void RemoconClient::key_sending(char key)
{
    memset(w_buff, 0 ,256);
    w_buff[0] = key;
    int write_check = 0;
